...
int main(int argc, char **argv, char **env) {
    ...
    evalAndDump(tfp, top, i); // BEQ  R1, R2, -4        - Expect branch back 1 line -> 1111111 00010 00001 000 11101 1100011  -> fe 20 8e e3
    ...
    evalAndDump(tfp, top, i); // BNE  R1, R2, -4        - Expect branch back 1 line -> 1111111 00010 00001 001 11101 1100011  -> fe 20 9e e3
    evalAndDump(tfp, top, i); //                        - Expect R1 = 5
    evalAndDump(tfp, top, i); //                        - Expect no jump
    evalAndDump(tfp, top, i); // ADDI R1, R0, 0x3       - Expect R1 = 0x3           -> ...00011 00000 000 00001 0010011       -> 00 30 00 93
    evalAndDump(tfp, top, i); // ADDI R1, R1, 0x1       - Expect R1 = 0x4           -> ...00001 00001 000 00001 0010011       -> 00 10 80 93
    evalAndDump(tfp, top, i); // BLT  R1, R2, -4        - Expect branch back 1 line -> 1111111 00010 00001 100 11101 1100011  -> fe 20 ce e3
    evalAndDump(tfp, top, i); //                        - Expect R1 = 0x5
    evalAndDump(tfp, top, i); //                        - Expect no jump
    evalAndDump(tfp, top, i); // ADDI R1, R1, 0x1       - Expect R1 = 0x6           -> ...00001 00001 000 00001 0010011       -> 00 10 80 93
    evalAndDump(tfp, top, i); // ADDI R1, R1, -1        - Expect R1 = 0x5           -> 111111111111 00001 000 00001 0010011   -> ff f0 80 93
    evalAndDump(tfp, top, i); // BGE  R1, R2, -4        - Expect branch back 1 line -> 1111111 00010 00001 101 11101 1100011  -> fe 20 de e3
    evalAndDump(tfp, top, i); //                        - Expect R1 = 0x4
    evalAndDump(tfp, top, i); //                        - Expect no jump
    evalAndDump(tfp, top, i); // ADDI R1, R0, 0x3       - Expect R1 = 0x3           -> ...00011 00000 000 00001 0010011       -> 00 30 00 93
    evalAndDump(tfp, top, i); // ADDI R1, R1, 0x1       - Expect R1 = 0x4           -> ...00001 00001 000 00001 0010011       -> 00 10 80 93
    evalAndDump(tfp, top, i); // BLTU R1, R2, -4        - Expect branch back 1 line -> 1111111 00010 00001 110 11101 1100011  -> fe 20 ee e3
    evalAndDump(tfp, top, i); //                        - Expect R1 = 0x5
    evalAndDump(tfp, top, i); //                        - Expect no jump
    evalAndDump(tfp, top, i); // ADDI R1, R1, 0x1       - Expect R1 = 0x6           -> ...00001 00001 000 00001 0010011       -> 00 10 80 93
    evalAndDump(tfp, top, i); // ADDI R1, R1, -1        - Expect R1 = 0x5           -> 111111111111 00001 000 00001 0010011   -> ff f0 80 93
    evalAndDump(tfp, top, i); // BGEU R1, R2, -4        - Expect branch back 1 line -> 1111111 00010 00001 111 11101 1100011  -> fe 20 fe e3
    evalAndDump(tfp, top, i); //                        - Expect R1 = 0x4
    evalAndDump(tfp, top, i); //                        - Expect no jump
